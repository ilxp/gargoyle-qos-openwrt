...
-- 检查qosmon进程是否在运行（更可靠的方法）
function is_qosmon_running()
    -- 方法1：使用pidof检查（最可靠）
    local pid = util.exec("pidof qosmon 2>/dev/null | awk '{print $1}'")
    if pid and pid:gsub("%s+", "") ~= "" then
        return true, pid:gsub("%s+", "")
    end
    
    -- 方法2：使用ps检查（精确匹配）
    local output = util.exec("ps | grep -w qosmon | grep -v grep | grep -v '/bin/sh' | head -1 | awk '{print $1}'")
    if output and output:gsub("%s+", "") ~= "" then
        return true, output:gsub("%s+", "")
    end
    
    -- 方法3：检查PID文件
    local pid_file = "/var/run/qosmon.pid"
...
function get_qosmon_args()
    local args = ""
    local ps_output = util.exec("ps | grep -v grep | grep -w qosmon | head -1")
...
function generate_qosmon_args()
    ...
    -- 1. 添加基本选项
    -- 注意：qosmon可能不需要 -b 参数，后台运行由init脚本处理
    -- 但我们保留它以防qosmon需要
...
function status_display.cfgvalue()
    -- 首先检查进程是否在运行
    local running, pid = is_qosmon_running()
    
    -- 然后获取配置中的enabled状态
    local config_enabled = uci:get("qos_gargoyle", "qosmon", "enabled") or "0"
...
function bandwidth_info.cfgvalue(self, section)
    ...
    return string.format("下载: %d Kbps (%d Mbps) | 上传: %d Kbps (%d Mbps)<br>qosmon启动参数: <code>%s</code>", 
...
ping_limit = adv_s:taboption("congestion", Value, "ping_limit", translate("自定义Ping限制 (ms)"),
    translate("自定义ping时间限制。如果指定，qosmon将使用此值作为延迟阈值，否则自动测量。"))
...
adv_desc.cfgvalue = function()
    ...
    <ul style="margin: 5px 0; padding-left: 20px;">
        ...
        <li>修改内部参数需要重新启动qosmon服务才能生效</li>
...
function m.on_after_save(self)
    ...
    if config.enabled == "1" then
        ...
        -- 启用开机启动
        sys.call("rm -f /etc/rc.d/S??qosmon 2>/dev/null")
        sys.call("ln -sf /etc/init.d/qosmon /etc/rc.d/S99qosmon 2>/dev/null")
        
        -- 直接使用qosmon命令启动，而不是通过init脚本
        local output_file = "/tmp/qosmon_start.log"
        local error_file = "/tmp/qosmon_error.log"
        ...
        -- 构建启动命令
        local start_cmd = "/usr/sbin/qosmon " .. args .. " > " .. output_file .. " 2>" .. error_file .. " &"
        ...
        if running then
            ...
            if error_msg ~= "未知错误" then
                ...
                m.message = m.message .. "<br><small>请检查/usr/sbin/qosmon是否存在且有执行权限</small>"
            end
        end
    else
        -- 禁用服务
        ...
        -- 禁用开机启动
        sys.call("rm -f /etc/rc.d/S??qosmon 2>/dev/null")
        sys.call("rm -f /etc/rc.d/K??qosmon 2>/dev/null")
        
        -- 检查是否停止成功
        os.execute("sleep 1")
        local running, _ = is_qosmon_running()
        if running then
            m.message = translate("⚠ ACC服务停止失败，请手动执行: killall -9 qosmon")
        else
