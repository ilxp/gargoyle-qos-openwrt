...
本文档介绍 OpenWrt 下 Gargoyle QoS 系统提供的六种流量整形算法实现模块。每个模块均基于 Linux TC 和 nftables 构建，支持上传/下载双向整形，并提供丰富的可调参数。此外，系统还包含一个**高性能的主动拥塞控制组件 `qosacc`**，用 C 语言实现，用于动态监测和调整带宽，进一步提升网络体验。

## 一、模块概览

| 算法组合                | 调度器 | 队列管理 | 适用场景                     |
|-------------------------|--------|----------|------------------------------|
| CAKE                    | CAKE   | CAKE     | 简单公平、多核硬件           |
| CAKE + DSCP 分类        | CAKE   | CAKE     | 智能分类、无需手动规则       |
...
- 每个方向最多支持 16 个类别（标记位空间限制），建议一般 3-4 个即可。

## 二、高性能主动拥塞控制组件 `qosacc`

`qosacc` 是 Gargoyle QoS 系统的核心主动拥塞控制组件，具有以下特点：

- **纯 C 语言实现**：运行效率极高，对路由器 CPU 占用极低。
- **动态带宽调整**：实时监测网络延迟和丢包情况，自动调整发送速率，避免拥塞崩溃。
- **智能阈值算法**：基于 ping 目标 IP 的响应时间，动态计算最佳带宽利用率。
- **与 TC 队列协同**：配合 CAKE、FQ_CODEL 等队列算法，进一步优化 Bufferbloat 问题。
- **配置灵活**：支持自定义 ping 目标、阈值和调整步长，适应不同网络环境。

启用 `qosacc` 后，系统能够：
- 在轻度拥塞时主动降速，保持低延迟；
- 在带宽空闲时快速恢复，充分利用链路容量；
- 显著改善游戏、VoIP 等实时应用的体验。
