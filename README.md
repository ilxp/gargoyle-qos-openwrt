# Qos_Gargoyle 算法模块说明
 ...
 Gargoyle QoS 是系统OpenWrt下提供的六种流量整形算法实现模块。每个模块均基于 Linux TC 和 nftables 构建，支持上传/下载双向整形，并提供丰富的可调参数。此外，系统还包含一个**高性能的主动拥塞控制组件 `qosacc`**，用 C 语言实现，用于动态监测和调整带宽，进一步提升网络体验。
