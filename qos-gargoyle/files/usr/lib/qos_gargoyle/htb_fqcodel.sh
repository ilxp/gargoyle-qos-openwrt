...
# version=2.13 - 整合rule.sh标记分配，添加conntrack恢复规则，修正调用顺序

# ========== 全局配置常量 ==========
: ${CONFIG_FILE:=qos_gargoyle}
: ${LOCK_DIR:=/var/run/htb_qos.lock}           # 并发锁目录
...
: ${QOS_RUNNING_FILE:=/var/run/htb_qos.running}   # 运行标记文件
...
load_required_modules() {
    local missing=0
    for mod in ifb sch_htb sch_fq_codel; do
...
check_already_running() {
    if [ -f "$QOS_RUNNING_FILE" ]; then
        ...
        if [ -n "$pid" ] && kill -0 "$pid" 2>/dev/null; then
            qos_log "ERROR" "HTB QoS 已经在运行中 (PID: $pid)"
            return 1
        else
...
init_htb_upload() {
    ...
    # 标记已在 init_htb_fqcodel_qos 中分配，这里直接创建类
    local class_index=2
    upload_class_mark_list=""
...
show_htb_status() {
    ...
    echo "===== HTB-FQ_CODEL QoS 状态报告 (v2.13) ====="
    echo "时间: $(date)"
    ...
    echo "上传QoS: $([ $upload_active -eq 1 ] && echo "已启用 (HTB+fq_codel)" || echo "未启用")"
    echo "下载QoS: $([ $download_active -eq 1 ] && echo "已启用 (HTB+fq_codel)" || echo "未启用")"
