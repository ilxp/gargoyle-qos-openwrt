...
# 创建HTB下载类别（优化：确保rate至少1kbit，统一使用局部IFB变量）
create_htb_download_class() {
    ...
    # 添加IPv6过滤器（使用 filter_prio + 100）- 统一与上传方向的计算逻辑
    if [ "$class_mark" != "0x0" ]; then
        local base_prio=100
...
show_htb_status() {
    ...
    tc qdisc show dev "$qos_ifb" 2>/dev/null | grep -q "htb" && download_active=1
