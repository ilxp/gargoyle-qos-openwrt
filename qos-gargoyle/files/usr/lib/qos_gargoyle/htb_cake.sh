#!/bin/bash
# HTB_CAKE算法实现模块
# 版本: 3.0.2 - 与 rule.sh 3.0.8 协同优化，修复 PID 检查、eval 安全隐患、burst 计算
...
# 锁标志（由 rule.sh 管理，不再重复定义）
HAVE_LOCK=0

# 确保使用 rule.sh 中的锁函数（已定义）
# acquire_lock, release_lock 等已在 rule.sh 中实现
...
calculate_htb_burst() {
    ...
    # 使用 ceil 确保 burst 不小于 MTU 的三倍
    local burst_kb=$(awk "BEGIN {printf \"%.0f\", ($rate + 7) / 8}")  # 向上取整
    # 确保最小值为 MTU 的 3 倍（转换为 KB）
...
create_htb_root_qdisc() {
    ...
    local default_opt=""
    # 验证默认类ID：必须为 2-16 之间（1 已被根类占用）
    if [[ -n "$default_classid" ]]; then
        if validate_number "$default_classid" "default_classid" 2 16; then
            default_opt="default $default_classid"
        else
            qos_log "ERROR" "无效的默认类ID: $default_classid (有效范围 2-16)"
            return 1
        fi
    else
...
setup_ingress_redirect() {
    ...
    # 使用 flower 匹配 IPv6 全球单播地址（dst_ip 已在验证中可用）
...
apply_htb_specific_rules() {
    ...
    # 注意：以下 meta priority 规则在 HTB 根队列下无效，仅用于可能的默认 pfifo_fast 调度器
