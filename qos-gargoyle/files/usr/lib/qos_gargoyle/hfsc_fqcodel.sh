...
# ========== HFSC 与 FQ_CODEL 专属配置加载 ==========
load_hfsc_fqcodel_config() {
...
# ========== 主初始化函数 ==========
init_hfsc_fqcodel_qos() {
    qos_log "INFO" "开始初始化HFSC+FQ_CODEL QoS系统"
    ...
    if ! check_already_running; then
        qos_log "ERROR" "HFSC+FQ_CODEL QoS 已经在运行中"
        release_lock
        return 1
    fi
    ...
    if ! load_hfsc_fqcodel_config; then
        qos_log "ERROR" "加载HFSC+FQ_CODEL配置失败"
        release_lock
        rm -f "$QOS_RUNNING_FILE"
        return 1
    fi
    ...
    if [ -z "$total_upload_bandwidth" ] || ! echo "$total_upload_bandwidth" | grep -q '^[0-9]\+$' || [ "$total_upload_bandwidth" -le 0 ]; then
        ...
        if ! init_hfsc_fqcodel_upload; then
    ...
    if [ -z "$total_download_bandwidth" ] || ! echo "$total_download_bandwidth" | grep -q '^[0-9]\+$' || [ "$total_download_bandwidth" -le 0 ]; then
        ...
        if ! init_hfsc_fqcodel_download; then
    ...
    if [ $upload_failed -eq 1 ] || [ $download_failed -eq 1 ]; then
        qos_log "ERROR" "HFSC+FQCODEL QoS 初始化部分失败"
        stop_hfsc_fqcodel_qos
        release_lock
        rm -f "$QOS_RUNNING_FILE"
        return 1
    ...
    setup_hfsc_enhance_chains
    apply_hfsc_specific_rules
    qos_log "INFO" "HFSC+FQ_CODEL QoS初始化完成"
    release_lock
    return 0
}
...
# ========== 停止函数 ==========
stop_hfsc_fqcodel_qos() {
    qos_log "INFO" "停止HFSC+FQ_CODEL QoS"
    ...
    qos_log "INFO" "HFSC+FQ_CODEL QoS停止完成"
    restore_main_config
    if $got_lock; then
        release_lock
    fi
...
show_hfsc_fqcodel_status() {
    ...
    echo "===== HFSC-FQ_CODEL QoS 状态报告 (v2.4.0) ====="
    echo "时间: $(date)"
...
# ========== 主入口 ==========
main_hfsc_fqcodel_qos() {
    local action="$1"
    case "$action" in
        "start")
            if ! init_hfsc_fqcodel_qos; then
                qos_log "ERROR" "HFSC+FQ_CODEL QoS启动失败"
                exit 1
            fi
            ;;
        "stop")
        ...
        "restart")
            ...
            sleep 1
            if ! init_hfsc_fqcodel_qos; then
                qos_log "ERROR" "HFSC+FQ_CODEL QoS重启失败"
                exit 1
            fi
            ;;
        "status")
        ...
        *)
            ...
            echo ""
            echo "命令:"
            echo "  start        启动HFSC+FQ_CODEL QoS"
            echo "  stop         停止HFSC+FQ_CODEL QoS"
            echo "  restart      重启HFSC+FQ_CODEL QoS"
            echo "  status       显示状态"
...
if [ "$(basename "$0")" = "hfsc_fqcodel.sh" ] || [ "$(basename "$0")" = "hfsc-fqcodel.sh" ]; then
    main_hfsc_fqcodel_qos "$1"
fi