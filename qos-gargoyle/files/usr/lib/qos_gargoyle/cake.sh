...
create_cake_root_qdisc() {
    ...
    if [ "$use_mq" = "1" ]; then
        ...
        if [ "$base_bw" -lt 1 ]; then
            base_bw=1
            remainder=0
            qos_log "WARN" "总带宽 ${bandwidth}kbit 小于队列数 ${queues}，每个队列将分配至少1kbit带宽，实际总带宽可能超过设定值"
        fi
        ...
        qos_log "INFO" "正在为 $device 创建普通CAKE队列..."
    ...
    # 注意：无论是普通 CAKE 还是 CAKE-MQ，根队列句柄均为 1:，因此可以安全添加 filter
    if [[ "$direction" == "upload" ]]; then
