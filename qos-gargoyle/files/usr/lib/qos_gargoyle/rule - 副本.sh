...
load_all_config_options() {
    ...
    class=$(uci -q get "${config_name}.${section_id}.class" 2>/dev/null)
    [ -n "$class" ] && ALL_OPTION_VARIABLES="${ALL_OPTION_VARIABLES}class "
    
    order=$(uci -q get "${config_name}.${section_id}.order" 2>/dev/null)
    [ -n "$order" ] && ALL_OPTION_VARIABLES="${ALL_OPTION_VARIABLES}order "
    
    enabled=$(uci -q get "${config_name}.${section_id}.enabled" 2>/dev/null)
    [ -n "$enabled" ] && ALL_OPTION_VARIABLES="${ALL_OPTION_VARIABLES}enabled "
    
    proto=$(uci -q get "${config_name}.${section_id}.proto" 2>/dev/null)
    [ -n "$proto" ] && ALL_OPTION_VARIABLES="${ALL_OPTION_VARIABLES}proto "
    
    srcport=$(uci -q get "${config_name}.${section_id}.srcport" 2>/dev/null)
    [ -n "$srcport" ] && ALL_OPTION_VARIABLES="${ALL_OPTION_VARIABLES}srcport "
    
    dstport=$(uci -q get "${config_name}.${section_id}.dstport" 2>/dev/null)
    [ -n "$dstport" ] && ALL_OPTION_VARIABLES="${ALL_OPTION_VARIABLES}dstport "
    
    connbytes_kb=$(uci -q get "${config_name}.${section_id}.connbytes_kb" 2>/dev/null)
    [ -n "$connbytes_kb" ] && ALL_OPTION_VARIABLES="${ALL_OPTION_VARIABLES}connbytes_kb "
    
    family=$(uci -q get "${config_name}.${section_id}.family" 2>/dev/null)
    [ -n "$family" ] && ALL_OPTION_VARIABLES="${ALL_OPTION_VARIABLES}family "
...
# 计算规则的综合优先级分数
# 公式: 综合分数 = class_priority * 1000 + rule_order
# 分数越小，优先级越高
calculate_composite_priority() {
    ...
    if [ -n "$direction" ]; then
        ...
        if [ "$direction" = "upload" ]; then
            class_priority=$(uci -q get "${CONFIG_FILE}.${class}.priority" 2>/dev/null || echo "999")
...
apply_enhanced_direction_rules() {
    ...
    if [ "$chain" = "filter_qos_egress" ]; then
        ...
        log "警告: 未知链类型: $chain，将不进行方向特定的优先级计算"
    fi
    ...
    log "按优先级排序后的规则: $sorted_rule_list"
    ...
    for rule_name in $sorted_rule_list; do
        ...
        local class="" order=""
        load_all_config_options "$CONFIG_FILE" "$rule_name"
        local class_priority=$(uci -q get "${CONFIG_FILE}.${class}.priority" 2>/dev/null || echo "999")
        log "  $rule_name -> 类[$class:prio$class_priority] + 规则[order:${order:-999}] = 总分:$score"
    done
    log "====================="
    
    # 按优先级顺序应用规则
    for rule_name in $sorted_rule_list; do
        log "应用规则: $rule_name (优先级顺序)"
...
apply_single_enhanced_rule() {
    ...
    # 加载规则配置
    local class="" family="" proto="" srcport="" dstport="" connbytes_kb=""
    load_all_config_options "$CONFIG_FILE" "$rule_name"
    ...
    local class_mark=""
    if [ "$chain" = "filter_qos_egress" ]; then
        class_mark=$(get_classname_mark "$class" "$upload_class_mark_list" "upload")
    else
        class_mark=$(get_classname_mark "$class" "$download_class_mark_list" "download")
    fi
...
create_priority_dualstack_filter() {
    ...
    local parent="$2"
    local class_id="$3"
    local class="$4"  # 新增：类别名称
    local mark="$5"
    local mask="$6"
    
    # 从配置获取该类别的priority
    local class_priority=$(uci -q get "${CONFIG_FILE}.${class}.priority" 2>/dev/null)
    class_priority=${class_priority:-100}  # 默认值
    
    # 使用class_priority作为TC过滤器优先级
    tc filter add dev "$dev" parent "$parent" protocol ip \
        prio $class_priority handle ${mark}/$mask fw flowid "$class_id"
    
    tc filter add dev "$dev" parent "$parent" protocol ipv6 \
        prio $((class_priority + 1)) handle ${mark}/$mask fw flowid "$class_id"
}
