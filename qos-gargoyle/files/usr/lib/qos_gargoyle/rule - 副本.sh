...
generate_ack_limit_rules() {
    ...
    qos_log "INFO" "ACK 限速丢包模式: $drop_mode ($([ "$drop_mode" = "random" ] && echo "随机丢包" || echo "直接丢包"))"
...
add rule inet ${NFT_TABLE} filter_qos_egress $addr_match meta length < 100 tcp flags ack \\
    limit rate over ${xfast_rate}/second add @qos_xfst_ack${set_suffix} { $final_key } \\
    counter $drop_action_xfast

# fast
add rule inet ${NFT_TABLE} filter_qos_egress $addr_match meta length < 100 tcp flags ack \\
    limit rate over ${fast_rate}/second add @qos_fast_ack${set_suffix} { $final_key } \\
    counter $drop_action_fast

# medium
add rule inet ${NFT_TABLE} filter_qos_egress $addr_match meta length < 100 tcp flags ack \\
    limit rate over ${med_rate}/second add @qos_med_ack${set_suffix} { $final_key } \\
    counter $drop_action_med

# slow
add rule inet ${NFT_TABLE} filter_qos_egress $addr_match meta length < 100 tcp flags ack \\
    limit rate over ${slow_rate}/second add @qos_slow_ack${set_suffix} { $final_key } \\
    counter $drop_action_slow
EOF
    }
...
add rule inet ${NFT_TABLE} filter_qos_egress meta l4proto tcp ct state established meta nfproto ipv4 \\
    limit rate over ${rate}/second burst ${burst} packets add @qos_slow_tcp { ct id . ct direction } \\
    ...
    if [[ "$udp_action" == "mark" ]]; then
        ...
        if [[ -n "$udp_upload_class" ]]; then
            ...
            if [[ -z "$upload_mark" ]]; then
                qos_log "WARN" "UDP 限速: 指定的上传类 '$udp_upload_class' 不存在，将使用最低优先级类"
                upload_mark=""
            fi
        fi
        if [[ -z "$upload_mark" ]]; then
            ...
            if [[ -n "$lowest_upload_mark" ]]; then
                ...
                qos_log "ERROR" "UDP 限速: 无法获取有效的上传标记，功能将被禁用"
                return
            fi
        fi
        ...
        if [[ -n "$udp_download_class" ]]; then
            ...
            if [[ -z "$download_mark" ]]; then
                qos_log "WARN" "UDP 限速: 指定的下载类 '$udp_download_class' 不存在，将使用最低优先级类"
                download_mark=""
            fi
        fi
        if [[ -z "$download_mark" ]]; then
            ...
            if [[ -n "$lowest_download_mark" ]]; then
                ...
                qos_log "ERROR" "UDP 限速: 无法获取有效的下载标记，功能将被禁用"
                return
            fi
        fi
...
# 若 IP 已在惩罚集合中，直接标记
add rule inet ${NFT_TABLE} filter_qos_egress oifname \"$wan_if\" meta l4proto udp \\
    ip saddr @udp_per_ip_upload \\
    meta mark set $upload_mark ct mark set (ct mark & 0xffff0000) | $upload_mark counter

# 若不在集合中，检查速率，超限则加入集合并标记当前包
add rule inet ${NFT_TABLE} filter_qos_egress oifname \"$wan_if\" meta l4proto udp \\
    ip saddr limit rate over ${udp_rate}/second add @udp_per_ip_upload { ip saddr timeout ${udp_timeout}s } \\
    meta mark set $upload_mark ct mark set (ct mark & 0xffff0000) | $upload_mark counter

add rule inet ${NFT_TABLE} filter_qos_egress oifname \"$wan_if\" meta l4proto udp \\
    ip6 saddr @udp_per_ip_upload_v6 \\
    meta mark set $upload_mark ct mark set (ct mark & 0xffff0000) | $upload_mark counter

add rule inet ${NFT_TABLE} filter_qos_egress oifname \"$wan_if\" meta l4proto udp \\
    ip6 saddr limit rate over ${udp_rate}/second add @udp_per_ip_upload_v6 { ip6 saddr timeout ${udp_timeout}s } \\
    meta mark set $upload_mark ct mark set (ct mark & 0xffff0000) | $upload_mark counter

# 下载方向
add rule inet ${NFT_TABLE} filter_qos_ingress iifname \"$wan_if\" meta l4proto udp \\
    ip daddr @udp_per_ip_download \\
    meta mark set $download_mark ct mark set (ct mark & 0xffff) | $download_mark counter

add rule inet ${NFT_TABLE} filter_qos_ingress iifname \"$wan_if\" meta l4proto udp \\
    ip daddr limit rate over ${udp_rate}/second add @udp_per_ip_download { ip daddr timeout ${udp_timeout}s } \\
    meta mark set $download_mark ct mark set (ct mark & 0xffff) | $download_mark counter

add rule inet ${NFT_TABLE} filter_qos_ingress iifname \"$wan_if\" meta l4proto udp \\
    ip6 daddr @udp_per_ip_download_v6 \\
    meta mark set $download_mark ct mark set (ct mark & 0xffff) | $download_mark counter

add rule inet ${NFT_TABLE} filter_qos_ingress iifname \"$wan_if\" meta l4proto udp \\
    ip6 daddr limit rate over ${udp_rate}/second add @udp_per_ip_download_v6 { ip6 daddr timeout ${udp_timeout}s } \\
    meta mark set $download_mark ct mark set (ct mark & 0xffff) | $download_mark counter"
...
add rule inet ${NFT_TABLE} filter_qos_egress oifname \"$wan_if\" meta l4proto udp \\
    ip saddr @udp_per_ip_upload counter drop

add rule inet ${NFT_TABLE} filter_qos_egress oifname \"$wan_if\" meta l4proto udp \\
    ip saddr limit rate over ${udp_rate}/second add @udp_per_ip_upload { ip saddr timeout ${udp_timeout}s } counter drop

add rule inet ${NFT_TABLE} filter_qos_egress oifname \"$wan_if\" meta l4proto udp \\
    ip6 saddr @udp_per_ip_upload_v6 counter drop

add rule inet ${NFT_TABLE} filter_qos_egress oifname \"$wan_if\" meta l4proto udp \\
    ip6 saddr limit rate over ${udp_rate}/second add @udp_per_ip_upload_v6 { ip6 saddr timeout ${udp_timeout}s } counter drop

# 下载方向
add rule inet ${NFT_TABLE} filter_qos_ingress iifname \"$wan_if\" meta l4proto udp \\
    ip daddr @udp_per_ip_download counter drop

add rule inet ${NFT_TABLE} filter_qos_ingress iifname \"$wan_if\" meta l4proto udp \\
    ip daddr limit rate over ${udp_rate}/second add @udp_per_ip_download { ip daddr timeout ${udp_timeout}s } counter drop

add rule inet ${NFT_TABLE} filter_qos_ingress iifname \"$wan_if\" meta l4proto udp \\
    ip6 daddr @udp_per_ip_download_v6 counter drop

add rule inet ${NFT_TABLE} filter_qos_ingress iifname \"$wan_if\" meta l4proto udp \\
    ip6 daddr limit rate over ${udp_rate}/second add @udp_per_ip_download_v6 { ip6 daddr timeout ${udp_timeout}s } counter drop"
    fi
...
# ========== 批量客户端检测（基于源 IP 连接建立速率，默认放在链尾，作为加强和补充） ==========
create_bulk_client_rules() {
    ...
    if [[ $use_conn_limit -eq 1 ]]; then
        ...
        if ! nft $add_or_insert rule inet ${NFT_TABLE} filter_forward "ct state new ct mark == 0 jump qos_bulk_conn_detect" 2>&1 | logger -t qos_gargoyle; then
    ...
    if [[ $use_byte_limit -eq 1 ]]; then
        ...
        if ! nft $add_or_insert rule inet ${NFT_TABLE} filter_forward "ct state established ct mark == 0 jump qos_bulk_byte_detect" 2>&1 | logger -t qos_gargoyle; then
    ...
    # ========== 上行标记规则 ==========
    # 条件：低16位为0（即未被高优先级标记），且源IP在批量集合中。
    # 设置 meta mark = upload_mark，并设置 ct mark 的低16位 = upload_mark（保留高16位不变）。
    # 注意：此规则不会覆盖已被高吞吐检测链设置的高优先级标记（因为高优先级标记的低16位非0）。
    if ! nft $add_or_insert rule inet ${NFT_TABLE} filter_forward oifname "$qos_interface" \
        "(ct mark & 0xffff) == 0" ip saddr @qos_bulk_clients4 \
        meta mark set $upload_mark \
        ct mark set "(ct mark & 0xffff0000) | $upload_mark" 2>&1 | logger -t qos_gargoyle; then
    ...
    # ========== 下行标记规则 ==========
    # 条件：高16位为0（即未被下载标记），且目的IP在批量集合中。
    # 设置 meta mark = download_mark，并设置 ct mark 的高16位 = download_mark（保留低16位不变）。
    # 注意：此规则不会覆盖已设置的高16位（例如来自其他模块的下载标记），但若高16位为0则设置。
    if ! nft $add_or_insert rule inet ${NFT_TABLE} filter_forward iifname "$qos_interface" \
        "(ct mark & 0xffff0000) == 0" ip daddr @qos_bulk_clients4 \
        meta mark set $download_mark \
        ct mark set "(ct mark & 0xffff) | $download_mark" 2>&1 | logger -t qos_gargoyle; then
        qos_log "WARN" "批量客户端检测: 添加下行 IPv4 标记规则 (高16位未标记) 失败"
    fi
    if ! nft $add_or_insert rule inet ${NFT_TABLE} filter_forward iifname "$qos_interface" \
        "(ct mark & 0xffff0000) == 0" ip6 daddr @qos_bulk_clients6 \
        meta mark set $download_mark \
        ct mark set "(ct mark & 0xffff) | $download_mark" 2>&1 | logger -t qos_gargoyle; then
        qos_log "WARN" "批量客户端检测: 添加下行 IPv6 标记规则 (高16位未标记) 失败"
    fi
...
create_high_throughput_service_rules() {
    ...
    if ! nft $add_or_insert rule inet ${NFT_TABLE} filter_forward "ct state new ct mark == 0 jump qos_htp_new_conn" 2>&1 | logger -t qos_gargoyle; then
    ...
    # 注意：外层条件 ct mark == 0 确保只有未被任何动态分类标记的连接进入此链。
    # 这实现了折中：一旦连接被批量客户端标记为低优先级（低16位非0），就不会再被高吞吐检测升级。
    # 但若连接在未被标记前就达到高吞吐，则会被设置高优先级标记，且后续批量检测不会覆盖（因为低16位已非0）。
    if ! nft $add_or_insert rule inet ${NFT_TABLE} filter_forward "ct state established ct mark == 0 jump qos_throughput_detect" 2>&1 | logger -t qos_gargoyle; then
        qos_log "WARN" "高吞吐服务检测: 添加已建立连接跳转规则失败"
    fi

    # ========== 上行字节速率检测及标记 ==========
    # 条件：服务已在集合中，且字节速率超限，且当前 ct mark == 0（即未被任何标记）。
    # 设置 meta mark = upload_mark（高优先级），并设置 ct mark 的低16位 = upload_mark。
    if ! nft add rule inet ${NFT_TABLE} qos_throughput_detect meta nfproto ipv4 \
        ...
        meta mark set $upload_mark \
        ct mark set "(ct mark & 0xffff0000) | $upload_mark" 2>&1 | logger -t qos_gargoyle; then
        qos_log "WARN" "高吞吐服务检测: 添加 IPv4 字节速率检测规则失败"
    fi
    if ! nft add rule inet ${NFT_TABLE} qos_throughput_detect meta nfproto ipv6 \
        ...
        meta mark set $upload_mark \
        ct mark set "(ct mark & 0xffff0000) | $upload_mark" 2>&1 | logger -t qos_gargoyle; then
        qos_log "WARN" "高吞吐服务检测: 添加 IPv6 字节速率检测规则失败"
    fi

    # ========== 下行标记规则（修复：添加字节速率检查） ==========
    # 条件：高16位为0（未被下载标记），源IP+端口在服务集合中，且该连接的字节速率超限。
    # 设置 meta mark = download_mark，并设置 ct mark 的高16位 = download_mark。
    # 注意：下行流量以源 IP+端口作为服务标识，与上行规则对称。
    if ! nft $add_or_insert rule inet ${NFT_TABLE} filter_forward iifname "$qos_interface" \
        "(ct mark & 0xffff0000) == 0" ip saddr . th sport . meta l4proto @qos_high_throughput_services4 \
        ip saddr . th sport . meta l4proto limit rate over ${min_bytes} bytes/second \
        meta mark set $download_mark \
        ct mark set "(ct mark & 0xffff) | $download_mark" 2>&1 | logger -t qos_gargoyle; then
        qos_log "WARN" "高吞吐服务检测: 添加下行 IPv4 标记规则 (高16位未标记) 失败"
    fi
    if ! nft $add_or_insert rule inet ${NFT_TABLE} filter_forward iifname "$qos_interface" \
        "(ct mark & 0xffff0000) == 0" ip6 saddr . th sport . meta l4proto @qos_high_throughput_services6 \
        ip6 saddr . th sport . meta l4proto limit rate over ${min_bytes} bytes/second \
        meta mark set $download_mark \
        ct mark set "(ct mark & 0xffff) | $download_mark" 2>&1 | logger -t qos_gargoyle; then
        qos_log "WARN" "高吞吐服务检测: 添加下行 IPv6 标记规则 (高16位未标记) 失败"
    fi
...
setup_dynamic_classification() {
    # 确定动态分类优先级（first/last），默认 last（使用已加载的全局变量）
    : ${DCLASSIFY_PRIO:=last}
    local add_or_insert="add"
    [[ "$DCLASSIFY_PRIO" == "first" ]] && add_or_insert="insert"
    qos_log "INFO" "动态分类链规则将使用优先级: $DCLASSIFY_PRIO (${add_or_insert})"

    qos_log "信息" "初始化动态分类链..."

    # 创建必要的 nftables 链（如果不存在）
    ...
    # 添加跳转规则
    if ! nft $add_or_insert rule inet ${NFT_TABLE} filter_forward ct state established jump qos_established_connection 2>&1 | logger -t qos_gargoyle; then
        qos_log "WARN" "动态分类: 添加 filter_forward 跳转规则失败"
    fi
    if ! nft $add_or_insert rule inet ${NFT_TABLE} filter_input ct state established jump qos_established_connection 2>&1 | logger -t qos_gargoyle; then
        qos_log "WARN" "动态分类: 添加 filter_input 跳转规则失败"
    fi
    if ! nft $add_or_insert rule inet ${NFT_TABLE} filter_output ct state established jump qos_established_connection 2>&1 | logger -t qos_gargoyle; then
        qos_log "WARN" "动态分类: 添加 filter_output 跳转规则失败"
    fi

    if ! nft $add_or_insert rule inet ${NFT_TABLE} filter_input "ct mark == 0" jump qos_dynamic_classify 2>&1 | logger -t qos_gargoyle; then
        qos_log "WARN" "动态分类: 添加 filter_input 动态分类规则失败"
    fi
    if ! nft $add_or_insert rule inet ${NFT_TABLE} filter_output "ct mark == 0" jump qos_dynamic_classify 2>&1 | logger -t qos_gargoyle; then
        qos_log "WARN" "动态分类: 添加 filter_output 动态分类规则失败"
    fi
    if ! nft $add_or_insert rule inet ${NFT_TABLE} filter_forward "ct mark == 0" jump qos_dynamic_classify 2>&1 | logger -t qos_gargoyle; then
        qos_log "WARN" "动态分类: 添加 filter_forward 动态分类规则失败"
    fi
    if ! nft $add_or_insert rule inet ${NFT_TABLE} filter_forward "ct mark == 0" jump qos_dynamic_classify_reply 2>&1 | logger -t qos_gargoyle; then
        qos_log "WARN" "动态分类: 添加 filter_forward 回复方向动态分类规则失败"
    fi

    # 调用子函数（全局开关已在 apply_enhanced_features 中判断）
    local bulk_ret=0
    local htp_ret=0
    # 高吞吐检测规则优先，确保其能覆盖批量检测
