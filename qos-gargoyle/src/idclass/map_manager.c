...
struct idclass_class_entry {
    ...
    int class_id;          /* 关联的 class_id（来自标记文件） */
    uint32_t mark_value;   /* 标记值（如 1,2,4,8...） */
};
...
/* 辅助映射：类名 -> (class_id, mark_value) 从标记文件加载 */
struct class_mark_map {
    const char *name;
    int class_id;
    uint32_t mark;
} class_mark_entries[IDCLASS_MAX_CLASS_ENTRIES];
static int class_mark_count = 0;
...
static void idclass_map_free_entry(struct idclass_map_entry *e) {
    ...
    /* 释放 DNS 条目中的正则表达式 */
    if (e->data.id == CL_MAP_DNS && e->data.addr.dns.pattern[0] == '/')
        regfree(&e->data.addr.dns.regex);
    free(e);
}
...
/* 从标记文件加载类名到 class_id 和 mark 的映射 */
static void load_class_marks_mapping(void) {
    /* 释放旧的 name 字符串，避免内存泄漏 */
    for (int i = 0; i < class_mark_count; i++)
        free((void *)class_mark_entries[i].name);
    class_mark_count = 0;
    ...
    char line[256];
    while (fgets(line, sizeof(line), fp) && class_mark_count < IDCLASS_MAX_CLASS_ENTRIES) {
        char *p = line;
        while (*p == ' ' || *p == '\t') p++;
        if (*p == '#' || *p == '\n' || *p == '\0')
            continue;

        char direction[16], class_name[64];
        unsigned int mark;
        if (sscanf(p, "%15[^:]:%63[^:]:%u", direction, class_name, &mark) != 3) {
            fprintf(stderr, "Invalid line in class_marks: %s", p);
            continue;
        }

        int class_id = -1;
        if (sscanf(class_name, "uclass_%d", &class_id) != 1 &&
            sscanf(class_name, "dclass_%d", &class_id) != 1) {
            fprintf(stderr, "Cannot extract class_id from class name: %s\n", class_name);
            continue;
        }

        if (class_id < 0 || class_id >= IDCLASS_MAX_CLASS_ENTRIES) {
            fprintf(stderr, "class_id %d out of range\n", class_id);
            continue;
        }

        /* 提取真正的类名（去掉 uclass_ 或 dclass_ 前缀） */
        char *underscore = strchr(class_name, '_');
        const char *real_name = underscore ? underscore + 1 : class_name;

        /* 存储映射（同一类可能上传下载都有，但 class_id 相同，mark 可能不同，我们取上传方向的 mark） */
        int found = -1;
        for (int i = 0; i < class_mark_count; i++) {
            if (class_mark_entries[i].class_id == class_id) {
                found = i;
                break;
            }
        }
        if (found == -1) {
            found = class_mark_count++;
            class_mark_entries[found].class_id = class_id;
            class_mark_entries[found].name = strdup(real_name);
        }
        if (strcmp(direction, "upload") == 0)
            class_mark_entries[found].mark = mark;
        else if (strcmp(direction, "download") == 0 && class_mark_entries[found].mark == 0)
            class_mark_entries[found].mark = mark; // 优先使用 upload 的 mark
    }
    fclose(fp);
}

/* 填充 prio_class_up/down 映射，动态从 map_class 查找 class_id */
static void update_prio_maps(void) {
    int up_fd = map_manager_get_fd_internal(CL_MAP_PRIO_CLASS_UP);
    int down_fd = map_manager_get_fd_internal(CL_MAP_PRIO_CLASS_DOWN);
    if (up_fd < 0 || down_fd < 0)
        return;

    /* 优先级级别 0-3 对应的类名（固定） */
    const char *prio_names[4] = { "realtime", "video", "normal", "bulk" };
    uint32_t class_ids[4] = { 0, 0, 0, 0 };
    /* 从 map_class 中查找每个优先级对应的 class_id */
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < ARRAY_SIZE(map_class); j++) {
            if (map_class[j] && strcmp(map_class[j]->name, prio_names[i]) == 0) {
                class_ids[i] = j;
                break;
            }
        }
        /* 如果仍然未找到，使用硬编码 fallback（假设 class_id 与索引一致） */
        if (class_ids[i] == 0) {
            if (strcmp(prio_names[i], "realtime") == 0) class_ids[i] = 0;
            else if (strcmp(prio_names[i], "video") == 0) class_ids[i] = 1;
            else if (strcmp(prio_names[i], "normal") == 0) class_ids[i] = 2;
            else if (strcmp(prio_names[i], "bulk") == 0) class_ids[i] = 3;
			ULOG_WARN("Class '%s' not found in map_class, using fallback class_id %d\n",
              prio_names[i], class_ids[i]);
        }
    }

    for (uint32_t prio = 0; prio < 4; prio++) {
        bpf_map_update_elem(up_fd, &prio, &class_ids[prio], BPF_ANY);
        bpf_map_update_elem(down_fd, &prio, &class_ids[prio], BPF_ANY);
    }
}

/* 更新 class_mark map */
static void update_class_mark_map(void) {
    int fd = map_manager_get_fd_internal(CL_MAP_CLASS_MARK);
    if (fd < 0) return;

    /* 如果标记文件未提供任何条目，则使用默认值 */
    if (class_mark_count == 0) {
        uint32_t default_marks[4] = { 1, 2, 4, 8 };  // realtime, video, normal, bulk 的默认 mark
        for (int i = 0; i < 4; i++) {
            bpf_map_update_elem(fd, &i, &default_marks[i], BPF_ANY);
        }
        return;
    }

    for (int i = 0; i < class_mark_count; i++) {
        uint32_t class_id = class_mark_entries[i].class_id;
        uint32_t mark = class_mark_entries[i].mark;
        bpf_map_update_elem(fd, &class_id, &mark, BPF_ANY);
    }
}
...
static int idclass_map_create_class(struct blob_attr *attr) {
    ...
    /* 注意：ingress/egress DSCP 值在标记模式下不再使用，但保留解析以防将来 DSCP 模式 */
...
void map_manager_set_classes(struct blob_attr *val) {
    ...
    // 更新 prio_class_up/down 和 class_mark 映射
    load_class_marks_mapping();
    update_prio_maps();
    update_class_mark_map();
}
...
static void idclass_update_ip_conn(struct uloop_timeout *t) {
    ...
    // 安全清空 ip_conn_map：反复获取第一个 key 并删除
    while (bpf_map_get_next_key(fd, NULL, &cur_key) == 0) {
        bpf_map_delete_elem(fd, &cur_key);
    }
...
int map_manager_init(void) {
    ...
    /* 加载标记文件映射并更新 prio maps（不再调用 load_persistent_class_marks） */
    load_class_marks_mapping();
    update_prio_maps();
    update_class_mark_map();
