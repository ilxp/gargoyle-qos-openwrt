 ...
 * Fixed: direction detection via module_flags, IPv6 DSCP corrected.
 */
...
#ifndef IDCLASS_IP_ONLY
#define IDCLASS_IP_ONLY (1 << 1)
#endif

#ifndef IDCLASS_SET_DSCP
                                           ...
                                           struct idclass_flow_config *cfg)
{
    ...
    } else if ((mask & FEATURE_RATIO) && stats->down_bytes == 0) {
        score_bulk += cfg->weight_ratio_bulk;
    }
...
static __always_inline void ipv6_set_dscp(struct __sk_buff *skb, __u32 offset, __u8 dscp)
{
    ...
    traffic_class = (__u8 *)ip6h;
    __u8 old = (traffic_class[0] << 4) | ((traffic_class[1] & 0xF0) >> 4);
    if ((old & 0xFC) == dscp) return;

    __u8 new = (old & 0x03) | (dscp << 2);
    traffic_class[0] = (traffic_class[0] & 0x0F) | (new >> 4);
    traffic_class[1] = (traffic_class[1] & 0x0F) | ((new & 0x0F) << 4);
}
...
int classify(struct __sk_buff *skb)
{
    ...
    if (dscp & IDCLASS_DSCP_CLASS_FLAG) {
        ...
        struct idclass_class *tmp = bpf_map_lookup_elem(&class_map, &key);
        if (tmp && (tmp->flags & IDCLASS_CLASS_FLAG_PRESENT)) {
            class = tmp;
        }
    }
