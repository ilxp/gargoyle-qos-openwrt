...
/* fallback 定义，确保 IDCLASS_INGRESS 可用（如果头文件未提供） */
...
/* 前向声明（必须在 VLIST_TREE 之前） */
static void interface_update_cb(struct vlist_tree *tree,
                                struct vlist_node *node_new,
                                struct vlist_node *node_old);
static void interface_stop_one(struct idclass_iface *iface);
...
static int cmd_add_ingress(struct idclass_iface *iface, bool eth) {
    ...
    /* DNS 重定向必须优先于 BPF 过滤器，因此使用更低的优先级数值（更早匹配） */
    int dns_prio = prio - 4;   /* 例如 0x10c */
    if (dns_prio < 0) dns_prio = 0;

    ofs = prepare_filter_cmd(buf, sizeof(buf), iface->ifname, dns_prio++, true, false);
    ...
    /* 添加主 BPF 过滤器（优先级稍高，数值比 DNS 重定向大） */
