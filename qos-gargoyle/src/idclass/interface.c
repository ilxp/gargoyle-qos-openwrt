...
static inline const char *idclass_iface_name(struct idclass_iface *iface)
{
...
static bool
iface_config_equal(struct idclass_iface *if1, struct idclass_iface *if2)
{
...
static void
iface_config_set(struct idclass_iface *iface, struct blob_attr *attr)
{
...
static const char *
interface_ifb_name(struct idclass_iface *iface)
{
...
cmd_add_bpf_filter(const char *ifname, int prio, bool egress, bool eth)
{
	...
	suffix = idclass_get_program(!egress * idclass_INGRESS + !eth * idclass_IP_ONLY, &prog_fd);
	if (!suffix)
		return -1;
...
static int
cmd_add_qdisc(struct idclass_iface *iface, const char *ifname, bool egress, bool eth)
{
	struct idclass_iface_config *cfg = &iface->config;
...
static int
cmd_add_ingress(struct idclass_iface *iface, bool eth)
{
	...
	APPEND(buf, ofs, " protocol ip u32 match ip sport 53 0xffff "
			 "flowid 1:1 action mirred egress redirect dev " idclass_DNS_IFNAME);
...
static void
interface_clear_qdisc(struct idclass_iface *iface)
{
	char buf[64];
	int i;
	...
	for (i = 0; i < 6; i++) {
		prepare_filter_cmd(buf, sizeof(buf), iface->ifname, idclass_PRIO_BASE + i, false, false);
		idclass_run_cmd(buf, true);
	}

	prepare_filter_cmd(buf, sizeof(buf), iface->ifname, idclass_PRIO_BASE, false, true);
	idclass_run_cmd(buf, true);
...
static void
interface_start(struct idclass_iface *iface)
{
	struct ifreq ifr = {};
	bool eth;
...
static void
interface_stop(struct idclass_iface *iface)
{
	if (!iface->ifname[0] || !iface->active)
		return;
...
static void
interface_set_config(struct idclass_iface *iface, struct blob_attr *config)
{
...
static void
idclass_iface_check_device(struct idclass_iface *iface)
{
	const char *name = idclass_iface_name(iface);
	int ifindex;
...
static void
idclass_iface_check_interface(struct idclass_iface *iface)
{
	const char *name = idclass_iface_name(iface);
	char ifname[IFNAMSIZ];

	if (idclass_ubus_check_interface(name, ifname, sizeof(ifname)) == 0) {
...
static void
__idclass_iface_status(struct blob_buf *b, struct idclass_iface *iface)
{
	void *c;

	c = blobmsg_open_table(b, idclass_iface_name(iface));
...
void idclass_iface_status(struct blob_buf *b)
{
	...
	vlist_for_each_element(&devices, iface, node)
		__idclass_iface_status(b, iface);
	blobmsg_close_table(b, c);
...
static int
idclass_nl_error_cb(struct sockaddr_nl *nla, struct nlmsgerr *err,
		   void *arg)
{
...
static void
__idclass_iface_get_device(struct blob_buf *b, struct idclass_iface *iface)
{
	if (!iface->ifname[0] || !iface->active)
		return;
...
void idclass_iface_get_devices(struct blob_buf *b)
{
	...
	vlist_for_each_element(&devices, iface, node)
		__idclass_iface_get_device(b, iface);
	vlist_for_each_element(&interfaces, iface, node)
		__idclass_iface_get_device(b, iface);
}
