 ...
 */
#include "common.h"
#include <linux/if_packet.h>      // 提供 struct sockaddr_ll
#include <net/if.h>
...
static int pkt_pull_name(struct packet *pkt, const void *hdr, char *dest) {
    ...
    if (dest) {
        ...
        len = dn_skipname(pkt->buffer, pkt->buffer + pkt->len);
    }
...
/* 内部函数：跳过 IPv6 扩展头，返回下一个协议号和载荷偏移 */
static int skip_ipv6_extensions(struct packet *pkt, int proto) {
    int max_loop = 10;
    while (max_loop-- > 0) {
        switch (proto) {
        ...
        case IPPROTO_AH:
        case IPPROTO_ESP: {
            struct ipv6_opt_hdr *opth = pkt->buffer;
            if (pkt->len < sizeof(*opth))
                return -1;
            int hdr_len = (opth->hdrlen + 1) * 8;
            /* 确保扩展头长度不超过剩余数据长度 */
            if (hdr_len > pkt->len)
                return -1;
            if (!pkt_pull(pkt, hdr_len))
                return -1;
            proto = opth->nexthdr;
            break;
        }
...
static void idclass_dns_packet_cb(struct packet *pkt) {
    ...
    case ETH_P_IPV6:
        ...
        proto = ip6->nexthdr;
        /* 跳过 IPv6 扩展头 */
        next_proto = skip_ipv6_extensions(pkt, proto);
        if (next_proto < 0)
            return;
        proto = next_proto;
        break;
